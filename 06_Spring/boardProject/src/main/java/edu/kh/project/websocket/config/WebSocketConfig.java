...
@Configuration // 서버 실행시 메서드 모두 실행
@EnableWebSocket // 웹소켓 활성화 설정
@RequiredArgsConstructor
public class WebSocketConfig implements WebSocketConfigurer {

	// Bean으로 등록된 같거나 자식 타입 주입(SessionHandshakeInterceptor)
	...
	public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
		...
		registry.addHandler(testWebsocketHandler, "/testSock")
				...
				// 클라이언트 연결시 HttpSession을 가로채서 핸들러에게 전달
				.setAllowedOriginPatterns("http://localhost/", "http://127.0.0.1/", "http://192.168.50.207/")
				// 웹소켓 요청이 허용되는 IP, Domain 지정
				.withSockJS();
