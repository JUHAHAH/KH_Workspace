...
-- NULL을 ("KH_CMH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 칼럼명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULL만 복사됨
...
-- JOIN 후 원하는 컬럼만 테이블로 복사
CREATE TABLE EMPLOYEE_COPY_2 AS
...
-- 9. 제약조건 추가
-- ALTER TABLE 테이블명 ADD [CONSTRAINT 제약조건명] 제약조건(컬러명)
...
-- EMPLOYEE TABLE의 JOB_CODE 외래키 제약조건 추가
-- 참조 테이블 JOB, 참조 컬럼은 JOB의 기본 키
